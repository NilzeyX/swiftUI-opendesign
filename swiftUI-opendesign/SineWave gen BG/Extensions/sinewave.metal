...
[[ stitchable ]] half4 sinewave(float2 position, half4 color, float2 size, float time) {
    ...
    for (half i = 0.0h; i < 10.0h; i++) {
        ...
        // Calculate a base blue value that varies from deep blue to lighter blue
        half blueBase = sin(i * 0.3h + time) * 0.3h + 0.7h;  // Range from 0.4 to 1.0 (lighter to deeper blue)
        
        // Calculate green component for teal/sky blue variations
        // More green for teal, less for navy
        half greenAmount = sin(i * 0.3h + 2.0h + time * 0.5h) * 0.3h + 0.3h;  // Range from 0.0 to 0.6
        
        half3 rainbow = half3(
            0.0h,                // No red for blue-only effect
            greenAmount * blueBase,  // Green varies with blue to create teal->sky->navy
            blueBase             // Blue base that varies in intensity
        );
